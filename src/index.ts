...
import type { HasDefaultExport } from '@detachhead/ts-helpers/dist/types/misc.js';
...
for (const fileName of readdirSync(rulesDir)) {
  if (path.extname(fileName).toLocaleLowerCase() === '.js') {
    rules[path.parse(fileName).name] =
      // eslint-disable-next-line @typescript-eslint/no-require-imports -- can't use await import here because top level
